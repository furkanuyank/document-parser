...
class GPTExtractor:
    ...
    def run_inference(self, api_url, input_data):
        ...
        if file_path.lower().endswith('.pdf'):
            return self._process_pdf(api_url, input_data)
        else:
            return self._process_non_pdf(api_url, input_data)
            
    def _process_pdf(self, api_url, input_data):
        ...
        results = self._process_pages(api_url, base64_images, input_data)

        shutil.rmtree(temp_dir, ignore_errors=True)
        return results, num_pages
    
    def _process_non_pdf(self, api_url, input_data):
        file_path = input_data[0]["file_path"]

        with open(file_path, "rb") as f:
            base64_image = base64.b64encode(f.read()).decode("utf-8")

        results = self._process_pages(api_url, [base64_image], input_data)
        return results, 1

    def _process_pages(self, api_url, base64_images, input_data):
        results = []
        ...
        if len(base64_images) <= 4:
            ...
            results.append(self._call_vllm_api(api_url, base64_images, prompt))
        else:
            batch_size = 4
            for i in range(0, len(base64_images), batch_size):
                ...
                results.append(self._call_vllm_api(api_url, batch_images, prompt))

        return results
        
    def _call_vllm_api(self, api_url, base64_images, prompt):
        print(prompt+"------\n")
        headers = {
